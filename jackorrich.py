...
class App:
    ...
    def update(self):
        ...
        # 操作判定
        if pyxel.btn(pyxel.KEY_LEFT) or pyxel.btnv(pyxel.GAMEPAD1_BUTTON_DPAD_LEFT,8,2):
            if -3 < self.dx:
                self.dx -= 1
            self.pldir = -1
        elif pyxel.btn(pyxel.KEY_RIGHT) or pyxel.btnv(pyxel.GAMEPAD1_BUTTON_DPAD_RIGHT,8,2):
            if self.dx < 3:
                self.dx += 1
            self.pldir = 1
        else:
